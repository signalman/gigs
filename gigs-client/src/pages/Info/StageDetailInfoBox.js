...
const StageDetailInfoBox = ({
  ...
  return (
    ...
    >
      ...
      <DetailInfoItem Icon={PeopleIcon} onEdit={openEditTargetDialog} editable={editable}>{((targetAge || Number(targetAge) === 0) && targetGender && (targetMinCount || Number(targetMinCount) === 0)) ? `${targetAge}대 ${targetGender} ${targetMinCount}인 이상` : '데이터를 추가해주세요.'}</DetailInfoItem>
