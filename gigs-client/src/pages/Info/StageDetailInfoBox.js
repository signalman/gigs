...
import { COLOR, ENUM, ifNull } from '../../utils/Constants';
...
const StageDetailInfoBox = ({
  ...
  return (
    ...
    >
      ...
      <DetailInfoItem Icon={PeopleIcon} onEdit={openEditTargetDialog} editable={editable}>{((targetAge || Number(targetAge) === 0) && targetGender !== 'DEFAULT' && (targetMinCount || Number(targetMinCount) === 0)) ? `${targetAge}대 ${ENUM[targetGender]} ${targetMinCount}인 이상` : '데이터를 추가해주세요.'}</DetailInfoItem>
      ...
      <DetailInfoItem Icon={LocationOnIcon} onEdit={openEditStageTypeDialog} editable={editable}>{stageType ? ENUM[stageType] : '데이터를 추가해주세요.'}</DetailInfoItem>
    </Box>
  );
};
